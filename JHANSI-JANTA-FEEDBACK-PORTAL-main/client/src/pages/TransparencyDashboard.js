...
const TransparencyDashboard = () => {
  ...
  const fetchInteractions = async (grievanceId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE}/api/transparency/interactions/${grievanceId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const markInteractionAsRead = async (eventId) => {
    try {
      ...
      console.warn('Read receipt update failed:', err.response?.data?.error || err.message);
    }
  };
  ...
  const fetchReport = async (grievanceId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE}/api/transparency/report/${grievanceId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchAnalytics = async (grievanceId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE}/api/transparency/analytics/grievance/${grievanceId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchDashboard = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE}/api/transparency/dashboard/roles`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const exportReport = async (format = 'json') => {
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        link.href = url;
        link.setAttribute('download', `grievance-${selectedGrievanceId}-report.csv`);
      ...
      <div className="transparency-content">
        ...
        {activeTab === 'overview' && (
          <div className="overview-section">
            <div className="info-box">
              ...
              <p>
                The Transparency Center provides a complete, real-time view of your grievance
                journey from submission to resolution. Track every step, every update, and
                every interaction between you, our admin team, and government officers.
              </p>
            </div>
            ...
            <div className="features-grid">
              <div className="feature-card">
                ...
                <p>See every update instantly as your grievance progresses through the system</p>
              </div>

              <div className="feature-card">
                <div className="feature-icon">🔄</div>
                <h4>Complete Timeline</h4>
                <p>View all changes, assignments, and communications in chronological order</p>
              </div>

              <div className="feature-card">
                <div className="feature-icon">👥</div>
                <h4>Role Transparency</h4>
                <p>See who is handling your grievance - admins, officers, and their departments</p>
              </div>

              <div className="feature-card">
                <div className="feature-icon">📊</div>
                <h4>Analytics</h4>
                <p>Get insights into resolution time, status changes, and participation metrics</p>
              </div>

              <div className="feature-card">
                <div className="feature-icon">📥</div>
                <h4>Export Reports</h4>
                <p>Download your complete grievance history in CSV or JSON format</p>
              </div>

              <div className="feature-card">
                <div className="feature-icon">🔐</div>
                <h4>Privacy Protected</h4>
                <p>Only see information relevant to you based on your role and permissions</p>
              </div>
            </div>
            ...
            <div className="getting-started">
              ...
              <ol>
                ...
                <li>Switch to different tabs to view timelines, reports, and analytics</li>
          ...
          <div className="timeline-section">
            {loading ? (
                  ...
                  <div key={event._id || index} className="timeline-item">
                    ...
                    <div className="timeline-content">
                      <div className="timeline-header">
                        ...
                        <span className="role-badge" style={{ backgroundColor: getEventColor(event.eventType) }}>
                          {event.performedBy.role}
                        </span>
                      </div>
                      <p className="timeline-actor">
                        By: <strong>{event.performedBy.name}</strong> ({event.performedBy.email})
                      </p>
                      <p className="timeline-message">{event.eventDescription?.message}</p>
                      {event.publicMessage && (
                      ...
                      {event.statusTransition && (
                        <div className="status-transition">
                          <span className="status-from">{event.statusTransition.from}</span>
                          <span className="transition-arrow">→</span>
                          <span className="status-to">{event.statusTransition.to}</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="no-data">
                <p>No timeline data available. Select a grievance to view its timeline.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="interactions-section">
            <div className="interactions-toolbar">
              <div className="interaction-stat">
                ...
                <strong>{interactionStats?.total || interactions.length || 0}</strong>
              </div>
              <div className="interaction-stat">
                <span>Unread</span>
            ...
            <div className="interaction-composer">
              ...
              <div className="composer-actions">
                {userRole === 'citizen' ? (
                  <div className="visibility-note">Visibility: Public (citizen messages are always public)</div>
                ) : (
                  <select
                  ...
                  >
                    ...
                    {userRole === 'admin' && <option value="admin-only">Admin Only</option>}
                  </select>
                )}
                <button
                  className="primary-btn"
            ...
            {loading ? (
              ...
              <div className="interactions-list">
                {interactions.map((interaction, index) => {
                  const normalizedRole = normalizeRole(interaction?.performedBy?.role);
                    ...
                    >
                      <div className="interaction-entry-header">
                        ...
                        <span className="event-chip">
                          {(interaction.eventType || 'MESSAGE').replace(/_/g, ' ')}
                        </span>
                        <span className={`visibility-chip visibility-${visibility}`}>
                          {visibility}
                        </span>
                      </div>
                      <p className="interaction-message">
                        {interaction.eventDescription?.message || interaction.publicMessage || 'No message'}
                      </p>
                      <div className="interaction-meta">
                        <span>
                          By {interaction?.performedBy?.name || 'Unknown'} ({interaction?.performedBy?.email || 'N/A'})
                        </span>
              ...
              <div className="no-data">
                <p>No interactions yet. Start the conversation to improve transparency.</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'report' && (
          <div className="report-section">
            <button className="secondary-btn" onClick={() => fetchReport(selectedGrievanceId)} disabled={loading}>
              ...
              <div className="report-container">
                <div className="report-header">
                  ...
                  <div className="report-metadata">
                    <span className="badge">{report.grievanceDetails.category}</span>
                    <span className="badge badge-status">{report.grievanceDetails.status}</span>
                    <span className="badge badge-priority">{report.grievanceDetails.priority}</span>
                  </div>
                </div>
                ...
                <div className="report-stats">
                  <div className="stat-card">
                    <h5>Total Events</h5>
                    <p className="stat-value">{report.statistics.totalEvents}</p>
                  </div>
                  <div className="stat-card">
                    <h5>Last Update</h5>
                    <p className="stat-value">
                      {new Date(report.statistics.lastUpdate).toLocaleDateString()}
                    </p>
                  </div>
                  {report.slaMetrics.firstResponseTime && (
                    <div className="stat-card">
                      ...
                      <p className="stat-value">
                        {new Date(report.slaMetrics.firstResponseTime).toLocaleDateString()}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="report-breakdown">
                  <div className="breakdown-section">
                    ...
                    <ul>
                      {Object.entries(report.statistics.eventsByType).map(([type, count]) => (
                        <li key={type}>
                          <span>{type}</span>
                  ...
                  <div className="breakdown-section">
                    ...
                    <ul>
                      {Object.entries(report.statistics.eventsByRole).map(([role, count]) => (
                        <li key={role}>
        ...
        {activeTab === 'analytics' && (
          <div className="analytics-section">
            <button className="secondary-btn" onClick={() => fetchAnalytics(selectedGrievanceId)} disabled={loading}>
            ...
            {analytics && (
              <div className="analytics-container">
                <div className="analytics-lifecycle">
                  ...
                  <div className="lifecycle-info">
                    <div className="lifecycle-item">
                      <span>Submitted:</span>
                      <strong>{new Date(analytics.lifecycle.submittedAt).toLocaleDateString()}</strong>
                    </div>
                    <div className="lifecycle-item">
                      <span>Last Updated:</span>
                      <strong>{new Date(analytics.lifecycle.lastUpdatedAt).toLocaleDateString()}</strong>
                    </div>
                    <div className="lifecycle-item">
                      <span>Days in System:</span>
                      <strong>{analytics.lifecycle.totalDaysInSystem} days</strong>
                    </div>
                  </div>
                </div>
                ...
                <div className="analytics-interactions">
                  ...
                  {Object.entries(analytics.roleInteractions).map(([role, data]) => (
                    <div key={role} className="interaction-card">
                      <h5>{role.toUpperCase()}</h5>
                      <p><strong>Interactions:</strong> {data.count}</p>
                      <p><strong>First:</strong> {new Date(data.firstInteraction).toLocaleDateString()}</p>
                      <p><strong>Last:</strong> {new Date(data.lastInteraction).toLocaleDateString()}</p>
                ...
                <div className="analytics-milestones">
                  ...
                  <ul className="milestones-list">
                    {analytics.keyMilestones.map((milestone, index) => (
                      <li key={index}>
                        <strong>{milestone.eventType}</strong> - {new Date(milestone.timestamp).toLocaleString()}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        {activeTab === 'dashboard' && userRole === 'admin' && (
          <div className="admin-dashboard-section">
            <button className="secondary-btn" onClick={fetchDashboard} disabled={loading}>
            ...
            {stats.summary && (
              <div className="dashboard-container">
                <div className="dashboard-summary">
                  <h3>System Activity - Last {stats.period?.days || 30} Days</h3>
                  <div className="summary-grid">
                    <div className="summary-card">
                      <h5>Total Activities</h5>
                      <p className="summary-value">{stats.summary.totalActivities}</p>
                    </div>
                    {Object.entries(stats.summary.byRole).map(([role, count]) => (
                  ...
                  <div className="role-analysis-grid">
                    {Object.entries(stats.roleAnalysis).map(([role, data]) => (
                      <div key={role} className="role-card">
                        ...
                        <p><strong>Avg/Day:</strong> {data.averageActivityPerDay}</p>
                        <div className="event-breakdown">
                          {Object.entries(data.eventTypes).slice(0, 3).map(([event, count]) => (
